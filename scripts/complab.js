async function submitReservation(confirmOverwrite = false) {
    ...
    // Validate seat selection for Tier 1 & 2 users
    if ((userTier === 1 || userTier === 2) && selectedSeats.length > 1) {
        errorMessage.textContent = "Error: You can only reserve one seat.";
        return;
    }
    ...
    for (let seat of selectedSeats) {
        try {
            // **First, Check for Lower-Tier Reservations**
            ...
            const response = await fetch("http://localhost:3000/reserve", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ room, roomType: "complab", date, timeslot, seat, username: user.username, userTier, confirmOverwrite })
            });
            ...
            if (response.ok) {
                ...
                if (userConfirmed) {
                    await submitReservation(true); // **Retry with confirmation enabled**
                    return; 
                } else {
                    return; // **Do not proceed if user declines**
                }
            } else {
                console.error(`Error for seat ${seat}: ${result.error}`);
                failedReservations++;
            }
        } catch (error) {
